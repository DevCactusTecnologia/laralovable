...
<head>
    <meta charset="utf-8" />
    <title>Quantitativo do analista - {{ $biomedicals['name'] }}</title>
...
<body style="font-family: Arial, Helvetica, sans-serif;">
    ...
    <div style="display: flex; justify-content: center;">
        <table style="border-collapse: collapse; border: 1px solid #000;">
            ...
            <tbody>
                @foreach ($biomedicals['exams_analyzeds'] as $name => $amount)
                    <tr>
                        <td style="border: 1px solid #000; padding: 3px 4px">{{ $name }}</td>
                        <td style="text-align: center; border: 1px solid #000; padding: 3px 4px">{{ $amount }}</td>
                    </tr>
                @endforeach
                <tr>
                    <td></td>
                    <td style="border: 1px solid #000; background-color: #CCC; font-weight: 600; text-align: center;">
                        {{ array_sum(array_values($biomedicals['exams_analyzeds'])) }}
                    </td>
                </tr>
            </tbody>
        </table>
