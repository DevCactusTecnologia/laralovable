...
<body style="font-family: Arial, Helvetica, sans-serif;">
    ...
    {{-- <div style="display: flex;">
        <div style="width: 50%;">
            <div style="font-size: 14px;">
                <div>
                    Unidade:&nbsp;<strong>{{ $unity['name'] }}</strong>
                </div>
            </div>
        </div>
    </div> --}}
    ...
    <div style="display: flex; justify-content: center; margin-bottom: 20px;">
        <table style="border-collapse: collapse; border: 1px solid #000;">
            ...
            <tbody>
                @foreach ($unitys['exams_analyzeds'] as $examName => $total)
                    <tr>
                        <td style="border: 1px solid #000; padding: 3px 4px">{{ $examName }}</td>
                        <td style="text-align: center; border: 1px solid #000; padding: 3px 4px">{{ $total }}</td>
                    </tr>
                @endforeach
                <tr>
                    <td></td>
                    <td style="text-align: center; border: 1px solid #000; padding: 3px 4px; background-color: #DDD; font-weight: bold;">
                        {{ array_sum($unitys['exams_analyzeds']) }}
                    </td>
                </tr>
            </tbody>
        </table>
