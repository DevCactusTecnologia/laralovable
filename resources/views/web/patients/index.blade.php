...
<body style="background-color: #FFF; padding: 20px;">
    ...
    <header class="d-flex align-items-center mb-2">
        <div class="mr-2">
            <img src="{{ asset('assets/images/brasao.png') }}" class="img-fluid" width="55px" alt="brasão">
        </div>
        <div class="d-flex flex-column justify-content-center">
            <strong>PREFEITURA DE COREMAS</strong>
    ...
    <div class="d-md-flex mb-4">
        Olá, &nbsp;<strong>{{ $appointments->first()->patient->first_name }}</strong>, 
        seja bem vind{{ $patient->gender == 'Female' ? 'a' : 'o' }} ao painel de atendimentos ao paciente.
    </div>
    ...
    <div id="accordion">
        ...
        @foreach ($appointments as $appointment)
            ...
            <div class="mb-2">
                ...
                >
                    <a href="#collapse-{{ $appointment->id }}" class="btn btn-link" data-toggle="collapse"
                        aria-expanded="true" aria-controls="collapse-{{ $appointment->id }}"
                        onclick="collapse(this, {{ $appointment->id }})"
                    >
                        <div class="d-flex align-items-center">
                            <div name="icon" class="mr-2" onclick="changeIcon({{ $appointment->id }})">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="#888" height="30" width="30" viewBox="0 0 48 48">
                                    <path d="m24 30.75-12-12 2.15-2.15L24 26.5l9.85-9.85L36 18.8Z"/>
                                </svg>
                            </div>
                            <div class="d-flex flex-column">
                                <div class="text-left">Protocolo: <strong>{{ $appointment->id }}</strong></div>
                                <div>Registrado em: <strong>{{ date('d/m/Y', strtotime($appointment->appointment_date)) }}</strong></div>
                            </div>
                        </div>
                    </a>
                    @if ($appointment->status == '1')
                        <a href="{{ route('patient.result.pdf', Crypt::encrypt($appointment->id)) }}" target="_blank">
                ...
                <div id="collapse-{{ $appointment->id }}" class="collapse" aria-labelledby="headingOne" data-parent="#accordion">
                            ...
                            <div class="accordion-group">
                                <div class="accordion-heading">
                                    <a class="accordion-toggle bg-light text-body collapsed d-flex align-items-center py-3" 
                                        data-toggle="collapse" data-parent="#accordionExams" href="#exam-{{ $appointment->id }}-{{ $exam->id }}"
                                    >
                                        ...
                                        <span class="ml-2">
                                            <span class="mr-1" style="font-weight: 600;">{{ $exam->name }}</span>
                                            <span>{{ $exam->pivot->status == '0' ? '(pendente)' : '' }}</span>
                                        </span>
                                    </a>
                                </div>
                                <div id="exam-{{ $appointment->id }}-{{ $exam->id }}" class="accordion-body collapse">
                                                ...
                                                <div class="content-result-exam" style="overflow-x: scroll;">
                                                    ...
                                                    @if ($exam->pivot->status == '1' && $exam->filters->isEmpty())
                                                        ...
                                                        <div class="d-flex align-items-center justify-content-end my-2">
                                                            ...
                                                            <div>{{ $biomedicalUser->first_name }}</div>
                                                        </div>
                                                    @endif

                                                    @if ($exam->pivot->status == '1' && $exam->filters->isNotEmpty())
