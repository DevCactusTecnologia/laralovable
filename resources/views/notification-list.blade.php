...
<div class="s-page">
        ...
        @component('components.breadcrumb')
            ...
            @slot('li_2') Notificações @endslot
        @endcomponent
        <!-- end page title -->
        <div class="row">
            <div class="col-xl-12">
                                                    ...
                                                    @if ($item->notification_type_id == 4)
                                                        <h6 class="mt-0 mb-1">
                                                            <a
                                                                ...
                                                                {{  $role == 'patient' ? '' : $item->invoice_user->patient->first_name . ' ' . $item->invoice_user->patient->first_name . ' ' }}
                                                            </a>
                                                            {{ $item->title }} por
                                                            @if ($item->user->roles[0]->slug == 'doctor')
                                                                <a
                                                                    href={{ url('doctor/' . $item->user->id) }}>{{ $item->user->first_name }}</a>
                                                            @elseif ($item->user->roles[0]->slug == 'patient')
                                                                <a
                                                                    href={{ url('patient/' . $item->user->id) }}>{{ $item->user->first_name }}</a>
                                                            @elseif ($item->user->roles[0]->slug == 'receptionist')
                                                                <a
                                                                    href={{ url('receptionist/' . $item->user->id) }}>{{ $item->user->first_name }}</a>
                                                            @else
                                                    ...
                                                    @else
                                                        <h6 class="mt-0 mb-1">
                                                            <a
                                                                ...
                                                                {{ $role == 'patient' ? '' : $item->appointment_user->patient->first_name }}
                                                            </a>
                                                            {{ $item->title }} por
                                                            @if ($item->user->roles[0]->slug == 'doctor')
                                                                <a
                                                            ...
                                                            @else
                                                                <a
                                                                    href="#!">{{ $item->user->first_name }}</a>
                                                            @endif
                                                        </h6>
                                                        <div class="font-sixe-12">
                                                            <p class="mb-0">Data do Atendimento:
                                                                {{ @$item->appointment_user->appointment_date . ' ' . @$item->appointment_user->timeSlot->from . ' ' . @$item->appointment_user->timeSlot->to }}
                                                            </p>
                                                        </div>
                                                    @endif
