@if ($exam->pivot->status == '0' || $exam->pivot->status == '1')
    <div class="d-md-flex justify-content-md-end" style="margin-top: -53px;">
        {{-- REMARCAR O RETESTE --}}
        <button type="button" class="btn" data-toggle="modal" data-target="#retest-{{ $exam->id }}"
            style="font-weight: 600; color: #ffa600; background-color: #ffe4b3" 
        >
            Reteste <i class="bx bx-columns"></i> 
        </button>
        <div class="modal fade" id="retest-{{ $exam->id }}" tabindex="-1" role="dialog" aria-hidden="true" aria-hidden="true">
            <div class="modal-dialog modal-md bg-white" role="document">
                <div class="modal-content">
                    <div class="modal-body p-4">
                        <form action="{{ route('appointments.result.retest', [$appointment->id, $exam->id]) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="d-md-flex flex-column justify-content-center">
                                ...
                                <span class="text-center mb-2">
                                    Deseja realmente encaminhar o exame <strong>{{ $exam->name }}</strong> para reteste?
                                </span>
                                <div class="mb-3">
                                    <label class="control-label">Motivo<span class="text-danger">*</span></label>
                                    <select class="form-control" name="motive" required>
                                        <option value="">Selecione</option>
                                        @foreach ($motives as $motive)
                                            <option value="{{ $motive->getName() }}">{{ $motive->getName() }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group mb-5">
                                    <label>Analista<span class="text-danger">*</span></label>
                            ...
                            <div class="d-flex justify-content-center modal-footer border-0 pt-0">
                                ...
                                <button type="submit" class="btn rounded-pill px-3" onclick="loader(this)"
        ...
        {{-- CANCELAR EXAMES --}}
        <button type="button" class="btn mx-2" data-toggle="modal" data-target="#cancel-{{ $exam->id }}"
            style="font-weight: 600; color: #c61010; background-color: #f9b1b1" 
        >
            Cancelar <i class="bx bxs-x-circle"></i>  
        </button>
        <div class="modal fade" id="cancel-{{ $exam->id }}" tabindex="-2" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-body p-4">
                        <form action="{{ route('appointments.result.cancel', [$appointment->id, $exam->id]) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="d-md-flex flex-column justify-content-center">
                                ...
                                <span class="text-center mb-2">
                                    Deseja realmente cancelar o exame <strong>{{ $exam->name }}</strong>?
                                </span>
                                <div class="mb-3">
                                    <label class="control-label">Motivo<span class="text-danger">*</span></label>
                                    <select class="form-control" name="motive" required>
                            ...
                            <div class="d-flex justify-content-center modal-footer border-0 pt-0">
                                ...
                                <button type="submit" class="btn btn-danger rounded-pill px-3" onclick="loader(this)">
                                    Cancelar
                                </button>
                            </div>
...
@else
        ...
        <div class="p-3 mt-3" style="color: #c61010; background-color: #f5bbbb;">
            <div class="d-md-flex justify-content-md-between align-items-md-center">
                <div>
                    ...
                    <div><strong>Motivo:</strong> {{ $exam->pivot->observation ?: 'NÃO INFORMADO' }}</div>
                </div>
    ...
    <div class="modal fade" id="restore-{{ $exam->id }}" tabindex="-2" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-body p-4">
                    <form action="{{ route('appointments.result.restore', [$appointment->id, $exam->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="d-md-flex flex-column justify-content-center">
                            ...
                            <input type="hidden" name="old_motive" value="{{ $exam->pivot->observation ?: 'NÃO INFORMADO' }}">
                        </div>
