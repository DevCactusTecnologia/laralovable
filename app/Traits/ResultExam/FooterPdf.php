...
trait FooterPdf
{
    private function footer($isPaginate = true)
    {
        PDF::setFooterCallback(function ($pdf) use ($isPaginate) {
            ...
            $pdf->Cell(w: $pdf->getPageWidth() - 20, h: 7, txt: 'RUA PROJETADA', border: 0, ln: 1, align: 'L', fill: false, link: '', stretch: 0);
            ...
            $pdf->SetFont('helvetica', '', 7);
            $pdf->Cell(w: $pdf->getPageWidth() - 20, h: 7, txt: 'Cabo Branco', border: 0, ln: 1, align: 'L', fill: false, link: '', stretch: 0);

            $pdf->SetXY(100, $pdf->getPageHeight() - 15);
            $pdf->Cell(w: $pdf->getPageWidth() - 20, h: 7, txt: 'RENICE CABRAL MIRANDA', border: 0, ln: 1, align: 'L', fill: false, link: '', stretch: 0);

            $pdf->SetXY(15, $pdf->getPageHeight() - 12);
            $pdf->SetFont('helvetica', 'B', 7);
            $pdf->Cell(w: $pdf->getPageWidth() - 20, h: 7, txt: 'Coremas/PB', border: 0, ln: 1, align: 'L', fill: false, link: '', stretch: 0);

            $pdf->SetXY(100, $pdf->getPageHeight() - 12);
            $pdf->SetFont('helvetica', '', 7);
            $pdf->Cell(w: $pdf->getPageWidth() - 20, h: 7, txt: 'CRBM/PB:14.005', border: 0, ln: 1, align: 'L', fill: false, link: '', stretch: 0);
