...
class Biomedical extends Model
{
    ...
    public static function getMap(int $id, string $date)
    {
        ...
        $biomedicalsList = json_decode(json_encode($biomedicalsList), true);
        $biomedicals = collect($biomedicalsList)->groupBy('guide_number')->toArray();

        $registers['patients_map_page_separated'][0] = '';
        $examWithMapPageSeparated = [];
        $examOthers = [];
        foreach ($biomedicals as $register) {
            ...
            foreach ($register as $index => $item) {
                ...
                if (in_array($item['exam_id'], Exam::withMapPageSeparated())) {
                    ...
                    $examOthers["{$item['patient_name']}|{$item['protocol']}"]['protocol'] = $item['protocol'];
                    $examOthers["{$item['patient_name']}|{$item['protocol']}"]['guide_number'] = $item['guide_number'];
                    $examOthers["{$item['patient_name']}|{$item['protocol']}"]['patient_name'] = $item['patient_name'];
                    $examOthers["{$item['patient_name']}|{$item['protocol']}"]['patient_cpf'] = Fill::maskCpf($item['patient_cpf']);
                    $examOthers["{$item['patient_name']}|{$item['protocol']}"]['patient_gender'] = $item['patient_gender'] == 'Female' ? 'F' : 'M';
                    $examOthers["{$item['patient_name']}|{$item['protocol']}"]['patient_age'] = Carbon::createFromDate($year, $month, $day)->diff(Carbon::now())->format('%y anos, %m meses, %d dias');

                    $examOthers["{$item['patient_name']}|{$item['protocol']}"]['exams'][] = 
                        // 'guide_number' => $item['guide_number'],
                        // 'patient_name' => $item['patient_name'],
                        // 'patient_cpf' => Fill::maskCpf($item['patient_cpf']),
